package com.spark.modules.wechat.smallProgram.service;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import com.spark.modules.common.exception.LycheeSparkException;
import com.spark.modules.common.utils.HttpUtils;
import com.spark.modules.wechat.smallProgram.entity.AccessTokenModel;
import com.spark.modules.wechat.smallProgram.entity.LoginSessionModel;
import com.spark.modules.wechat.smallProgram.enums.WechatApiEnum;
...
public class WechatApiService {
    ...
    public static AccessTokenModel getAccessToken(String appId, String secret) {
        ...
        String url = WechatApiEnum.API_PREFIX.getUrl()
                .concat(WechatApiEnum.GET_ACCESS_TOKEN_API.getUrl()) + "?grant_type=client_credential&appid=" + appId + "&secret=" + secret;
    ...
    public LoginSessionModel login(String appId, String secret, String code) {
        ...
        String res = HttpUtils.get(WechatApiEnum.API_PREFIX.getUrl()
                .concat(WechatApiEnum.LOGIN_API.getUrl()), params);
        if (StringUtils.isEmpty(res)) {
     ...
     * @return 结果
     */
    public String getUserPhoneNumber(String appId, String secret, String code) {
        Assert.hasText(appId,"appId不能为空");
        Assert.hasText(secret,"secret不能为空");
        Assert.hasText(code,"code不能为空");
        AccessTokenModel accessToken = getAccessToken(appId, secret);
        String url = WechatApiEnum.API_PREFIX.getUrl()
                .concat(WechatApiEnum.GET_USER_PHONE_API.getUrl())+ "?access_token="+accessToken.getAccessToken();
        Map<String, String> params = new HashMap<>();
    ...
    public static BufferedImage getQrCode(Map<String, Object> params) throws IOException {
        ...
        if (params.containsKey("appId") && params.containsKey("secret") && params.containsKey("path")){
            ...
            String url = WechatApiEnum.API_PREFIX.getUrl()
                    .concat(WechatApiEnum.GET_QR_CODE_API.getUrl())+ "?access_token="+accessToken.getAccessToken();
            // 清除 appId secret
            params.remove("appId");
